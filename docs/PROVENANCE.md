...
| 08 GitOps Prompts | *Load Prompts from GitHub Repo and auto-populate n8n expressions* | Fetch prompt text from GitHub, substitute variables, fail if a variable is missing | Versioned prompt registry with front-matter, commit SHA returned with every render, bundled fallback on fetch failure, prompt linting in CI |
